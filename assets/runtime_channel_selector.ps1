...
function Show-HardwareProfileDialog {
    ...
    $dialog.ClientSize = New-Object Drawing.Size(720, 500)
    $dialog.Font = New-Object Drawing.Font("Segoe UI", 9)

    $heading = New-Object Windows.Forms.Label
    $heading.Text = "Select the model profile and runtime version"
    ...
    $hardware.Text = "$gpuText`n$ramText`nRecommended model: $($recommended.label)`nRecommended runtime: $runtimeRecommendation"
    ...
    $runtimeLabel.Location = New-Object Drawing.Point(22, 210)
    ...
    $stableRadio.Location = New-Object Drawing.Point(24, 234)
    ...
    $newRadio.Location = New-Object Drawing.Point(24, 260)
    ...
    $runtimeHint.Text = "Auto-selection is applied for you. RTX 50-series GPUs default to New; RTX 30/40-series GPUs default to Stable. You can switch versions manually for testing or compatibility."
    $runtimeHint.Location = New-Object Drawing.Point(44, 286)
    ...
    $details.Location = New-Object Drawing.Point(22, 330)
    ...
    $ok.Location = New-Object Drawing.Point(474, 444)
    ...
    $updateDetails = {
        try {
            ...
            if ((Test-IsBlackwellGpu $snapshot.GpuName) -and $runtimeChannel -eq "stable") {
                $manualWarning = " | Warning: New is recommended for RTX 50 series."
            } elseif (-not (Test-IsBlackwellGpu $snapshot.GpuName) -and $runtimeChannel -eq "new") {
                $manualWarning = " | Manual test mode: driver 580+ is recommended."
            }
            $details.Text = "$(Get-ProfileSummaryText $profile)`nRuntime: $($runtime.label)$manualWarning`n$state"
            $ok.Enabled = $true
        } catch {
